...
Instructions: Once you have finished this assignment, render the document (Ctrl/Cmd-Shift-K or the Render button). Commit the qmd file and any other files you have changed to the repository and push your changes. In Canvas, submit a link to your github repository containing the updated files. Note: Submitting your link in Canvas lets me know that you are ready for me to grade your homework. You can commit half-finished or not-working homework to GitHub, and it is good practice to commit frequently (at least at the end of each work session).
...
Whether we are in R or python the script mode gives the opportunity to write the codes including data source in a separate text file and upload
the codes in R or python and run them.
As for interactive both in R and python, we have to type code in the console for R and the terminal for python before we run them. 
The drawback with the interactive mode is that we have to re-type the codes each time we want to run them.
Similary, for the command line there is advantage of adding comments to the codes and save them for future use.
In conclusion, the script mode is always prefer because it could be saved as text file even with data link and can be run time.
(FYI,not for grade)The script is called do-file in Stata and does exactly the same thing.
---
...
salary <- read.table("02-data.tsv", sep = "\t", header = T);
#Replace the dollar $ in salary column
salary[1, 2] <- 67872
library(ggplot2)
ggplot(salary, aes(x = Hire.Year, y = Salary, label = Name)) + 
  geom_label()
